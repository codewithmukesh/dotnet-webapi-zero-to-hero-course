...
public static class AuthEndpoints
{
        ...
        ITokenService tokenService)
    {
        ...
        // Resolved here so the response can show the caller what they hold.
        // Whether these actually travel INSIDE the token depends on
        // Permissions:Source in appsettings.json.
        var permissions = await permissionStore.GetForRolesAsync(roles);

        var (token, expiresAt) = tokenService.CreateToken(user, roles, userClaims, permissions);

        return Results.Ok(new AuthResponse(
            user.Id, user.Email!, roles, permissions.Order(), token, expiresAt));
    }
}
